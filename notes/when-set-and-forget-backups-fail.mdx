...
Recently, while working with a client, I uncovered a problem that illustrates why backups aren’t
just about having files - they’re also about having reliable processes around them.


The system in question has been running for years and consists of:
...
The client had expressed concerns about the ability to restore the system in the event of an outage. His
...
Since cron jobs don’t surface errors unless you explicitly wire them up, my first stop was to examine the container 
logs.
...
So instead of running daily at 10:00 AM, the job was being parsed incorrectly (look again at the errors in Step 3,
...
- But the backup script wasn't running due to the crontab format issue

A single extra field likely broke the entire backup process.
...
This makes the backup script run every 5 minutes. Not ideal for production, but good for testing.
...
would work. I made that update and rebuilt the container again.
...
I wrote up a summary of the issue in the JIRA ticket. In case this happens again we can fix it quickly.

The client was happy that this was resolved and documented.
...
> A backup process that isn’t observed is indistinguishable from one that’s broken.
...
- When was the last time someone actually restored a database backup?
